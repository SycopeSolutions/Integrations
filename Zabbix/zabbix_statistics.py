...
# Zabbix credentials
ZABBIX_URL = "http://192.168.1.46:8080/api_jsonrpc.php"
ZABBIX_USER = "Admin"
ZABBIX_PASSWORD = ""
...
# Custom Index name and history period
INDEX_NAME = "ZabbixStats"
HISTORY_PERIOD_MINUTES = 1440
...
def sycope_login():
    ...
    r = s.post(sycopeHost + "/npm/api/v1/login", json=payload, verify=False)
    data = r.json()

    if data["status"] == 200:
        print("Sycope API login successful. Proceeding...")
    else:
        # For debugging
        print("Sycope API login failed:")
        print(r.json())
...
def get_hostname_by_id(auth_token, host_id):
    payload = {
        "jsonrpc": "2.0",
        "method": "host.get",
...
# def get_items_id(auth_token, host_id, key_):
def get_items_id(auth_token, host_id):
    payload = {
        "jsonrpc": "2.0",
        "method": "item.get",
        "params": {
...
def get_history(auth_token, item_id, start_time, end_time, history_type=0):
    payload = {
        "jsonrpc": "2.0",
        "method": "history.get",
        "params": {
...
def main():
    ...
    startTime = datetime.now() - timedelta(minutes=HISTORY_PERIOD_MINUTES)
    ...
    for ip in TARGET_IPS:
        ...
        host_id = get_host_id_by_ip(auth_token, ip)
        if not host_id:
            continue

        hostname = get_hostname_by_id(auth_token, host_id)
        # items_id = get_items_id(auth_token, host_id, key_)
        items_id = get_items_id(auth_token, host_id)
        ...
        for metric_name, val_config in ITEM_KEYS.items():
            ...
            for key_ in val_config["values"]:
                if key_ not in items_ids_dict:
                    continue
                history = get_history(
                    auth_token, items_ids_dict[key_], start_time_int, end_time_int, history_type=0
                )
    ...
    r = s.post(sycopeHost + "/npm/api/v1/pipeline/run", json=payload, verify=False)
    job_run = r.json()
    ...
    for offset in range(job_run["data"]["total"] // off_size + 1):
        ...
        r = s.post(f"{sycopeHost}/npm/api/v1/pipeline/{job_run['jobId']}/data", json=payload, verify=False)
        chunk = r.json()
    ...
    if not df.is_empty():
        ...
        r = s.post(sycopeHost + "/npm/api/v1/index/inject", json=new_data, verify=False)
        data = r.json()
        if data['status'] == 200:
